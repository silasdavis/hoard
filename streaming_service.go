...
func (service *StreamingService) UnsealGet(grt *grant.Grant, send func(*api.Plaintext) error) error {
	...
	for _, ref := range refs {
		...
		err = decodePlaintext(data, ref.GetType(), service.grantService.Get, send, version.LatestGrantVersion)
		if err != nil {
			return err
		}
	}
...
func (service *StreamingService) Decrypt(send func(*api.Plaintext) error, recv func() (*api.ReferenceAndCiphertext, error)) error {
	for {
		...
		err = decodePlaintext(data, refAndCiphertext.Reference.GetType(), service.grantService.Get, send, version.LatestGrantVersion)
		if err != nil {
			return err
		}
	}
